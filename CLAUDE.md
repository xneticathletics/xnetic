...
### Navigation quirk: don't add custom `animation` to the HomeStack redirect screens

`SocialFeed`/`Shop`/`ShopManage`/`EventsList`/`EventsManage` in `HomeStack.tsx` are reached via a tabPress-redirect from `CustomTabBar` (`RoleTabs.tsx`) rather than being real tabs. Two separate attempts to give these a custom transition (`animation: "none"`, then `"fade"`) each broke back-button/gesture reliability on screens pushed from them, in a way that "worked once then failed" — a known-fragile interaction between native-stack v7 + react-native-screens + the New Architecture. Current state (no `animation` override, default slide transition) is intentional; re-adding one needs strong justification and thorough back-nav testing across all three affected sub-screens.
