...
// TrainingSessionsScreen/TrainingSessionFormScreen'de "bu antrenör hangi
// salon(lar) için antrenman ekleyebilir" sorusuna cevap verir.
export async function getMyAuthorizedVenueIds(): Promise<string[]> {
  const userId = await getCurrentAppUserId();
  if (!userId) return [];
  const { data, error } = await supabase.from("venue_coaches").select("venue_id").eq("coach_id", userId);
  if (error) throw error;
  return (data ?? []).map((r) => r.venue_id);
}
