...
// FitnessProgramBuilderScreen'in bireysel karşılığı — tek fark: kulüpteki
// gruplara sergilenmiyor (kişisel program, hedef grup/bildirim yok),
// bu yüzden "Programı Sonlandır" adımı sadece program adını soruyor.
export default function IndividualFitnessProgramBuilderScreen({ route, navigation }: Props) {
  const { athleteId } = route.params;
  const { scrollRef, handleFocus } = useKeyboardScroll();
  const headerHeight = useHeaderHeight();

  const [items, setItems] = useState<IndividualFitnessProgramItemInput[]>([]);
        ...
        {!finalizing ? (
          ...
          <>
            ...
            <TextInput
              ...
              placeholder="Örn. Yaz Kuvvet Programım"
              placeholderTextColor={colors.muted}
            />

            {error && <Text style={styles.errorText}>{error}</Text>}

            <TouchableOpacity style={styles.completeButton} onPress={handleSave} disabled={saving}>
              <Text style={styles.completeButtonText}>{saving ? "Kaydediliyor…" : "Programı Kaydet"}</Text>
            </TouchableOpacity>
          </>
        )}
      </ScrollView>
