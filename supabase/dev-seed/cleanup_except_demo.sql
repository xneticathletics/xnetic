...
do $clean$
declare
  ids uuid[] := array(select id from clubs where name <> 'X-NETIC Demo Kulübü');
  sa uuid;
begin
  ...
  delete from auth.users where id in (select auth_user_id from users where club_id = any (ids) and role <> 'super_admin');
  delete from clubs where id = any (ids);
  delete from users where club_id = any (ids) and role <> 'super_admin';
end
$clean$;
