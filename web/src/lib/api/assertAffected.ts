...
// fonksiyonu `.select()` hiç eklemeden çağırıyordu — yani RLS bir yazmayı
// engellediğinde ekran "başarılı" gösterip hiçbir şey değişmemiş oluyordu.
// Bu yardımcı, o satır sayısını (data.length) kontrol edip 0 ise anlamlı
// bir hata fırlatır. Mobildeki src/lib/api/assertAffected.ts ile birebir
// aynı — kasıtlı kopya.
